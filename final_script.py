...
def M(image_id):
    ...
    img = tiff.imread(inDir+'/sixteen_band/{}_M.tif'.format(image_id))
...
def predict_id(id, model, trs):
    ...
    rgb = tiff.imread('data/three_band/{}.tif'.format(id))
    rgb = np.rollaxis(rgb, 0, 3)
    rgb = cv2.resize(rgb,(img.shape[1], img.shape[0]), interpolation = cv2.INTER_CUBIC)
        # get our index for waterways
    CCCI = CCCI_index(np.rollaxis(img, 2, 0), rgb) #needed: unrolled M but rolled rgb
    print "np.count_nonzero(CCCI): ", np.count_nonzero(CCCI)

    binary = (CCCI > 0.11).astype(np.int8)
    water = np.zeros((960,960)).astype(np.float32)
    water[0:binary.shape[0],0:binary.shape[1]] = binary
    bnsum = np.sum(binary)
    print "np.sum(binary): " ,bnsum
    ...
    for i in range(0, 6):
        ...
        for j in range(tmp.shape[0]):
            ...
            prd[6, i * ISZ:(i + 1) * ISZ, j * ISZ:(j + 1) * ISZ]  = water[i * ISZ:(i + 1) * ISZ, j * ISZ:(j + 1) * ISZ]
            
            ttx = water[i * ISZ:(i + 1) * ISZ, j * ISZ:(j + 1) * ISZ]
            print "partial default class 7 pred shape and unique values:" , np.sum(tmp[j][6]>0.3)
            print "partial enhanced class 7 pred shape and unique values :" ,  np.sum(ttx)

    trs = [0.4, 0.1, 0.4, 0.3, 0.3, 0.5, 0.3, 0.6, 0.1, 0.1]
    for i in range(N_Cls):
        if i==6:
            if bnsum < 31000: #avoid noise
                prd[i] = np.zeros(prd[i].shape)
            prd[i] = prd[i].astype(np.bool)    
            continue

        prd[i] = prd[i] > trs[i]

    print "mask comparison "
    print "final class 7 pred shape, unique values and np.sum:" , prd[6].shape, np.unique(prd[6]) , np.sum(prd[6])
    print "final class 1 pred shape, unique values and np.sum" ,  prd[0].shape, np.unique(prd[0]), np.sum(prd[0])
...
def make_submit():
    ...
    print(df.head())
    df.to_csv('subm/42.csv', index=False)
